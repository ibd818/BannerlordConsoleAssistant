...
- 对物品、兵种、领地和物品词缀提供紧凑类型/效果筛选；筛选只缩小可输入下拉框的选项，不改变命令参数格式
- 候选项显示为“英文 ID/名称（中文名）”，生成命令始终使用英文原始值；任务名、部队名等随存档变化的目标仍可手动输入
- 一键复制命令到剪贴板
- 独立的 UTF-8 `commands.json`，支持不改源码扩展
- 独立的 UTF-8 `entities.json`，按目标游戏版本保存候选目录和实体元数据
- 深色中文界面，高 DPI 适配
...
└── data/entities.json     # 当前版本目标 ID、英文名、中文标签和选择器元数据
...
目录项的 `value` 是最终写入命令的原始值，`label` 只用于界面显示，`aliases` 用于英文 ID、英文名和中文名的包含匹配，`metadata` 用于候选项摘要、详情和筛选。当前版本会为兵种显示等级/文化/定位，为物品显示类别/子类/基础等级，为领地显示类型/初始阵营，并对静态资料和存档动态状态做出区分。兵种等级由目标版本模块 XML 的 `level` 字段按游戏 Tier 规则计算，不再根据兵种名称猜测；领地阵营是初始阵营，不代表当前存档归属。中文标签和元数据来自版本化数据，作为辅助参考；游戏内 `help` 和实际模块数据仍是命令是否可用的最终依据。
