...
目录项的 `value` 是最终写入命令的原始值，`label` 只用于界面显示，`aliases` 用于英文 ID、英文名和中文名的包含匹配，`metadata` 用于候选项摘要、详情和筛选。当前版本会为兵种显示等级/文化/定位，为物品显示类别/子类/基础等级，为领地显示类型/初始阵营，并对静态资料和存档动态状态做出区分。兵种中未由 ID 直接提供的等级会标记为第一版估算；领地阵营是初始阵营，不代表当前存档归属。中文标签和元数据来自版本化数据，作为辅助参考；游戏内 `help` 和实际模块数据仍是命令是否可用的最终依据。
